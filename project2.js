...
class MeshDrawer {
	...
	draw(trans) {
		...
		updateLightPos();
		gl.uniform3fv(this.lightSourceLoc, normalize([lightX, lightY, 10])); // Example light direction
...
const meshFS = `
			...
			void main()
			{
				...
				gl_FragColor = texture2D(tex, v_texCoord) * ((ambient/1.5) + dot(normal, -lightPos)); }
